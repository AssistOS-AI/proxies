...
export async function listKeys() {
  const { rows } = await query(`
    SELECT id, label, key_hint, rpm_limit, expires_at, is_revoked, last_used_at, created_at
    FROM api_keys ORDER BY created_at DESC
  `);
  return rows;
}

export async function createKey({ label, rpm_limit, expires_at, key }) {
  ...
  const { rows } = await query(`
    INSERT INTO api_keys (key_hash, encrypted_key, label, key_hint, rpm_limit, expires_at)
    VALUES ($1, $2, $3, $4, $5, $6)
    RETURNING id, label, key_hint, rpm_limit, expires_at, is_revoked, created_at
  `, [keyHash, encKey, label, keyHint, rpm_limit ?? 60, expires_at || null]);

  return { ...rows[0], key: rawKey };
}

export async function updateKey(id, fields) {
  const allowed = ['label', 'rpm_limit'];
  const sets = [];
  const params = [];
  let idx = 1;
