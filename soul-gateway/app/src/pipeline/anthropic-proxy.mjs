...
const log = createLogger('anthropic-proxy');

/**
 * Anthropic Messages API passthrough proxy (/v1/messages).
 * Authenticates, rate-limits, blacklist-checks, then proxies the raw
 * request/response to the upstream provider in Anthropic format.
 */
export async function anthropicProxy(req, res) {
  ...
  try {
    ...
    // 4. Blacklist scan (checkBlacklist handles both string and array content)
    if (body.messages) {
      const messagesToScan = [...body.messages];
      // Also scan system prompt if present (Anthropic puts it top-level)
      if (body.system) {
        if (typeof body.system === 'string') {
          messagesToScan.unshift({ role: 'system', content: body.system });
    ...
    // 8. Build upstream URL — baseURL may be a full endpoint path
    //    (e.g. .../v1/chat/completions or .../v1/messages). Replace everything
    //    from /v1/ onwards with /v1/messages to get the Anthropic messages endpoint.
    const parsed = new URL(baseURL);
    const v1Idx = parsed.pathname.indexOf('/v1/');
    const basePath = v1Idx >= 0 ? parsed.pathname.slice(0, v1Idx) : '';
    const upstreamUrl = parsed.origin + basePath + '/v1/messages';
    ...
    let result;
    if (body.stream) {
      result = await pipeAnthropicStream(upstreamRes, res, startTime);
    } else {
      result = await handleAnthropicNonStreaming(upstreamRes, res, startTime);
    }
    ...
    log.error('Anthropic proxy error', { error: err.message, stack: err.stack });
...
/**
 * Pipe upstream Anthropic SSE stream to client byte-for-byte,
 * while parsing SSE events to extract metrics for logging.
 */
async function pipeAnthropicStream(upstreamRes, clientRes, startTime) {
  ...
  try {
    ...
    while (true) {
      ...
      // Parse SSE events from the chunk for metrics extraction
      sseBuffer += text;
      ...
      for (const event of parsed) {
        ...
        try {
          ...
          if (event.type === 'message_start' && data.message?.usage) {
            ...
            if (data.usage?.output_tokens) usage.completion_tokens = data.usage.output_tokens;
          }
        } catch {
...
/**
 * Handle non-streaming Anthropic response: forward as-is, extract metrics.
 */
async function handleAnthropicNonStreaming(upstreamRes, clientRes, startTime) {
