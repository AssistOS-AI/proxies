...
(() => {
  ...
  function extractCodeBlocks(input, store) {
    ...
    return input.replace(/```([^\n`]*)\n([\s\S]*?)```/g, (_, lang, body) => {
      ...
      store[token] = `<pre class="sg-md-code"><code${lang ? ` data-lang="${escapeAttribute(lang.trim())}"` : ''}>${escapeHtml((body || '').replace(/\s+$/g, ''))}</code></pre>`;
      return `\n\n${token}\n\n`;
    });
  }
  ...
  function splitTableRow(row) {
    ...
    if (!trimmed) return [];
    let start = 0, end = trimmed.length;
  ...
  function tryRenderTable(block, state, processInline) {
    ...
    if (!headerCells.length || headerCells.length !== sepCells.length) return null;
    if (!sepCells.every(c => TABLE_SEPARATOR_RE.test(c.replace(/\s+/g, '')))) return null;
    const aligns = sepCells.map(parseAlignment);
    const alignAttr = a => a ? ` style="text-align:${a}"` : '';
    const head = headerCells.map((c, i) => `<th${alignAttr(aligns[i])}>${processInline(c)}</th>`).join('');
  ...
  function processInlineFactory(state) {
    return function processInline(text) {
      ...
      let w = text.replace(/`([^`]+)`/g, (_, code) => {
        const t = inlineToken();
        inlineStore[t] = `<code class="sg-md-inline-code">${escapeHtml(code)}</code>`;
        return t;
      });
      w = escapeHtml(w);
      w = w.replace(/\*\*([^*]+)\*\*/g, (_, b) => `<strong>${b}</strong>`);
      w = w.replace(/(^|\s)\*([^*]+)\*(?=\s|$)/g, (_, lead, it) => `${lead}<em>${it}</em>`);
      w = w.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (_, label, url) => {
        try {
          const u = new URL(url.trim(), window.location.origin);
          if (u.protocol === 'http:' || u.protocol === 'https:') {
            const t = state.linkTokenFactory();
            state.placeholders[t] = `<a href="${escapeAttribute(u.href)}" target="_blank" rel="noopener noreferrer">${escapeHtml(label)}</a>`;
            return t;
          }
        } catch {}
        return `${label} (${url})`;
      ...
      w = w.replace(/\n/g, '<br/>');
      for (const t of Object.keys(inlineStore)) w = w.split(t).join(inlineStore[t]);
  ...
  function renderMarkdown(src) {
    ...
    input = input.replace(/\n(?!\s*\n)(\s{0,3}[-*+]\s+)/g, (m, b, off, str) => {
      const prev = str.slice(0, off);
      const lastNl = prev.lastIndexOf('\n');
      const prevLine = lastNl === -1 ? prev : prev.slice(lastNl + 1);
      return (/^\s{0,3}[-*+]\s+/.test(prevLine) ? '\n' : '\n\n') + b;
    });
    input = input.replace(/\n(?!\s*\n)(\s{0,3}\d+\.\s+)/g, (m, b, off, str) => {
      const prev = str.slice(0, off);
      const lastNl = prev.lastIndexOf('\n');
      const prevLine = lastNl === -1 ? prev : prev.slice(lastNl + 1);
    ...
    let html = blocks.map(block => {
      ...
      if (tbl) return tbl;
      if (/^\s{0,3}[-*+]\s+/.test(t)) return renderList(t, 'ul', state, processInline);
      if (/^\s{0,3}\d+\.\s+/.test(t)) return renderList(t, 'ol', state, processInline);
      if (/^#{1,6}\s/.test(t)) {
        const lines = t.split('\n');
