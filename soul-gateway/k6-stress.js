...
const API_KEY  = __ENV.API_KEY  || 'sk-soul-eea0fd976aaa8a05712fe60b9db973727470e5edabaf31ca31953b65240e85a8';
...
const TOPICS = ['quicksort','recursion','TCP','DNS','hashing','caching','polymorphism','garbage collection'];
const THINGS = ['colors','planets','animals','fruits','rivers','metals','languages','instruments'];
const COUNTRIES = ['France','Japan','Brazil','Egypt','Canada','India','Norway','Chile','Kenya','Turkey'];
const SUBJECTS = ['code','rain','coffee','stars','trees','bugs','loops','time'];
const WORDS = ['ephemeral','ubiquitous','pragmatic','serendipity','entropy','latency','throughput','idempotent'];
const ACRONYMS = ['HTTP','DNS','TCP','API','REST','SQL','CSS','HTML','JWT','SSH'];
const PHRASES = ['hello','goodbye','thank you','good morning','how are you','welcome','cheers','good night'];
const LANGUAGES = ['Spanish','French','German','Japanese','Italian','Portuguese','Korean','Russian'];
...
function chatCompletion(model, stream) {
  ...
  if (ok) {
    ...
    modelErrors.add(1);
    if (res.status !== 200) {
      console.warn(`[${model}] status=${res.status} body=${res.body?.substring(0, 200)}`);
    }
  }
