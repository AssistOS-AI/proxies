...
export async function bootstrap() {
  ...
  try {
    const { registerPublicApiRoutes } = await import('./public-api/router.mjs');
    registerPublicApiRoutes(httpRouter, appCtx);
    log.info('public API routes registered');
  } catch (err) {
    log.error('public API routes failed', { error: err.message, stack: err.stack });
  }

  // 9c. Register management routes
  try {
    const { buildManagementRouter } = await import('./management/router.mjs');
    const { httpRouter: mgmtHttp, wsRouter: mgmtWs } = buildManagementRouter(appCtx);
    ...
    log.error('management routes failed', { error: err.message, stack: err.stack });
  }
