...
export function startBackgroundJobs(appCtx) {
  ...
  function schedule(name, intervalMs, fn) {
    const timer = setInterval(async () => {
      try {
        ...
        log.error(`background job failed: ${name}`, { error: err.message });
      }
    }, intervalMs);
    timer.unref();
  ...
  schedule('partition-maintenance', env.PARTITION_JOB_INTERVAL_MS, async () => {
    if (!appCtx.pool || !env.DATABASE_URL) return;
    const { ensurePartition, dropExpiredPartitions } = await import('../db/dao/audit-logs-dao.mjs');
    const now = new Date();
  ...
  schedule('token-refresh', env.TOKEN_REFRESH_INTERVAL_MS || 60_000, async () => {
    if (!appCtx.pool || !env.DATABASE_URL) return;
    if (!appCtx.services.oauthManager) return;
    const { listExpiringOAuth } = await import('../db/dao/provider-accounts-dao.mjs');
    const expiring = await listExpiringOAuth(appCtx.pool);
    for (const account of expiring) {
      try {
        await appCtx.services.oauthManager.refreshTokens(account.id, account.oauth_adapter_key);
      } catch (err) {
        log.warn('token refresh failed for account', { accountId: account.id, error: err.message });
      }
    }
  });

  // Quota reset sweep — reactivate accounts whose quota period has elapsed
  schedule('quota-reset-sweep', env.QUOTA_RESET_SWEEP_MS || 300_000, async () => {
    if (!appCtx.pool || !env.DATABASE_URL) return;
    const { sweepExpiredQuotas } = await import('../db/dao/provider-accounts-dao.mjs');
    ...
    if (accountPool) {
      if (restored.length > 0) {
        cleared = accountPool.clearExhaustions(restored.map((row) => row.id));
      }
