import { sendTextFrame, sendPing, sendCloseFrame, OPCODE_TEXT, OPCODE_CLOSE, OPCODE_PING, OPCODE_PONG, decodeFrame } from '../core/websocket-frame-codec.mjs';
...
export class BroadcastHub {
  ...
  startHeartbeat(intervalMs) {
    this._heartbeatTimer = setInterval(() => this._sendHeartbeats(), intervalMs);
  ...
  stop() {
    ...
    for (const [id, client] of this.wsClients) {
      try { sendCloseFrame(client.socket, 1001, 'server shutting down'); } catch {}
  ...
  addSseSubscriber(stream, filters = {}, soulSpecific = false) {
    ...
    this.sseClients.set(id, { stream, filters, soulSpecific, addedAt: Date.now() });
  ...
  addWsSubscriber(socket, filters = {}, soulSpecific = false) {
    const id = ++this._subscriberId;
    this.wsClients.set(id, { socket, filters, soulSpecific, addedAt: Date.now() });
  ...
  updateFilters(subscriberId, filters) {
    ...
    if (sse) { sse.filters = filters; return; }
  ...
  publish(logRow) {
    ...
    for (const [, client] of this.sseClients) {
      if (!matchesFilters(logRow, client.filters, client.soulSpecific)) continue;
    ...
    for (const [, client] of this.wsClients) {
      if (!matchesFilters(logRow, client.filters, client.soulSpecific)) continue;
      const payload = client.soulSpecific ? fullJson : redactedJson;
      try { sendTextFrame(client.socket, payload); } catch {}
    }
  }
  ...
  _handleWsFrame(subscriberId, frame) {
    if (frame.opcode === OPCODE_PING) {
      ...
      if (client) {
        try {
          const { encodeFrame } = { encodeFrame: (op, data) => {
    ...
    if (frame.opcode === OPCODE_CLOSE) {
      const client = this.wsClients.get(subscriberId);
      if (client) {
        try { sendCloseFrame(client.socket, 1000); } catch {}
        client.socket.destroy();
      }
  ...
  _sendHeartbeats() {
    ...
    for (const [, client] of this.wsClients) {
      try { sendPing(client.socket); } catch {}
    }
  }
}
...
function redactLogEntry(logRow) {
  const { request_payload, response_payload, request_headers, ...safe } = logRow;
  return safe;
}
