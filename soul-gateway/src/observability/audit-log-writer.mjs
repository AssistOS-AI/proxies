...
export class AuditLogWriter {
  ...
  async start(entry) {
    try {
      ...
      this.log.error('audit start write failed', { requestId: entry.request_id, error: err.message });
    }
  }
  ...
  async finalize(entry) {
    try {
      ...
      this.log.error('audit finalize write failed', { requestId: entry.request_id, error: err.message });
      return null;
    }
  }
}
