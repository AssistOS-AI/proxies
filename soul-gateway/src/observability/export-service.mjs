...
const CSV_COLUMNS = [
  'log_id', 'request_id', 'soul_id', 'agent_name', 'requested_model',
  'status', 'http_status', 'error_type', 'latency_ms', 'ttfb_ms',
  'input_tokens', 'output_tokens', 'total_tokens', 'total_cost_usd',
  'cache_hit', 'blocked', 'streaming', 'started_at', 'completed_at',
];
  ...
  async exportCsv(res, { from, to, soulId, model }) {
    ...
    while (true) {
      const { rows } = await auditDao.query(this.pool, {
        from, to, soul_id: soulId, model,
        limit: this.batchSize, offset,
        sort: 'started_at', order: 'asc',
      });

      for (const row of rows) {
        res.write(CSV_COLUMNS.map(col => escapeCsvField(row[col])).join(',') + '\n');
      }
