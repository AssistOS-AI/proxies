...
export function normalizeIncomingFormat(routeKind, body) {
  ...
  switch (routeKind) {
    ...
    default:
      throw new UnsupportedFormatError(`Unsupported ingress format: ${routeKind}`);
  }
}
...
function normalizeOpenAiChat(body) {
  ...
  return { messages: messages || [], model, stream: Boolean(stream), ...rest };
}
...
function normalizeAnthropicMessages(body) {
  ...
  if (system) {
    if (typeof system === 'string') {
      ...
      messages.push({ role: 'system', content: convertAnthropicContent(system) });
    }
  }
      ...
      return {
        ...
        content: typeof item.content === 'string'
          ...
          : Array.isArray(item.content)
            ? item.content.map(convertResponsesContentPart).filter(Boolean)
            : '',
      };
...
function convertResponsesContentPart(part) {
  ...
  switch (part.type) {
    ...
    case 'input_image':
      return {
        type: 'image_url',
        image_url: { url: part.image_url || (part.detail ? part.image_url : '') },
      };
    case 'input_audio':
