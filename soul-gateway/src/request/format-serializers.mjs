 ...
 */
export function serializeBufferedResponse(completion, responseFormat, requestId) {
...
export function serializeStreamChunk(chunk, responseFormat, requestId) {
  switch (responseFormat) {
    ...
    case 'anthropic_messages':
      return JSON.stringify(serializeAnthropicStreamChunk(chunk, requestId));
    case 'openai_responses':
      return JSON.stringify(serializeResponsesStreamChunk(chunk, requestId));
    default:
      return JSON.stringify(serializeOpenAiStreamChunk(chunk, requestId));
  }
}
...
function serializeOpenAiResponses(completion, requestId) {
  ...
  return {
    ...
    usage: completion.usage ? {
      input_tokens: completion.usage.prompt_tokens || 0,
      output_tokens: completion.usage.completion_tokens || 0,
      total_tokens: (completion.usage.prompt_tokens || 0) + (completion.usage.completion_tokens || 0),
    } : null,
  };
}
