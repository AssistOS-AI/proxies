...
import { parseUrl } from './path-router.mjs';
 ...
 * @param {object} httpRouter    - path router returned by createRouter()
...
export function createHttpServer(appCtx, httpRouter, wsRouter) {
    ...
    const server = createServer(async (req, res) => {
        ...
        if (!match && appCtx.services.managementHttpRouter) {
            match = appCtx.services.managementHttpRouter.match(
                req.method,
                pathname
            );
        }
    ...
    if (wsRouter) {
        server.on('upgrade', async (req, socket, head) => {
            ...
            if (!match && appCtx.services.managementWsRouter) {
                match = appCtx.services.managementWsRouter.match(
                    'GET',
                    pathname
                );
            }
