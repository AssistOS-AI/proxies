...
export function buildProviderLifecycleOptions(appCtx) {
  return {
    ...
    executorCatalog: appCtx.services.executorCatalog || null,
...
export async function upsertProviderApiKeyAccount({
  ...
  const existingAccounts = await accountsDao.listByProvider(appCtx.pool, providerId);
  const existing = existingAccounts.find((account) => account.auth_type === 'api_key');
