...
export async function handleCreateKey(ctx) {
  ...
  const plaintextKey = body.keyValue || (DEFAULTS.apiKeyPrefix + randomBytes(32).toString('hex'));
  ...
  const { hashApiKey, derivePepper } = await import('../runtime/security/api-key-auth.mjs');

  const pepper = derivePepper(appCtx.config.env);
  const keyHash = hashApiKey(plaintextKey, pepper);

  const encryptionKey = (await import('../runtime/security/encryption.mjs')).ensureEncryptionKey(appCtx.config.env);
  const { ciphertext: keyCiphertext, iv: keyIv, authTag: keyAuthTag } = encrypt(plaintextKey, encryptionKey);
...
export async function handleUpdateKey(ctx) {
  ...
  const allowed = ['label', 'rpmLimit', 'tpmLimit', 'dailyBudgetUsd', 'monthlyBudgetUsd', 'expiresAt', 'metadata'];
  const fields = {};
