...
export async function create(pool, {
  label, keyHash, keyCiphertext, keyIv, keyAuthTag, keyHint,
  rpmLimit = 60, tpmLimit = 100000,
  dailyBudgetUsd = null, monthlyBudgetUsd = null,
  ...
  const { rows } = await pool.query(
    `INSERT INTO ${TABLE}
     ...
     RETURNING *`,
    [label, keyHash, keyCiphertext, keyIv, keyAuthTag, keyHint,
     rpmLimit, tpmLimit, dailyBudgetUsd, monthlyBudgetUsd,
...
export async function list(pool, { status = null, limit = 100, offset = 0 } = {}) {
  if (status) {
    const { rows } = await pool.query(
...
export async function update(pool, id, fields) {
  ...
  const values = keys.map((k) => k === 'metadata' ? JSON.stringify(fields[k]) : fields[k]);
