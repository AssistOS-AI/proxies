 ...
 * completion, not an empty answer. A row whose policy sets
 * `lengthWithoutContentFails` — the free-model execution policy does, because
 * there the gateway chose the model — treats it as an empty answer so the
 * next child is tried. A reply that ended normally without content stays an
 * empty answer everywhere.
 *
...
describe('a reply cut off by the token limit before any content', () => {
    ...
    it('fails over as an empty answer under the free-model execution policy', async () => {
        const ctx = makeCtx({ ...FREE_MODEL_EXECUTION_POLICY });
