...
import {
    checkBudget,
    recordSpend,
} from '../../runtime/policy/budget-enforcer.mjs';
...
describe('SpendCache', () => {
    ...
    it('returns cached values after refresh', async () => {
        ...
        const mockPool = createMockPool(1.5, 25.0);
        await cache.refresh('key-1', mockPool);

        assert.equal(cache.getDailySpend('key-1'), 1.5);
        assert.equal(cache.getMonthlySpend('key-1'), 25.0);
    });
    ...
    it('returns null when cache is stale', async () => {
        ...
        const mockPool = createMockPool(1.0, 10.0);
        await cache.refresh('key-1', mockPool);

        assert.equal(cache.getDailySpend('key-1'), 1.0);
    ...
    it('optimistically adds cost via recordCost', async () => {
        ...
        cache.recordCost('key-1', 0.5);

        assert.equal(cache.getDailySpend('key-1'), 1.5);
        assert.equal(cache.getMonthlySpend('key-1'), 10.5);
    });
    ...
    it('resets daily spend', async () => {
        ...
        const mockPool = createMockPool(5.0, 50.0);
        await cache.refresh('key-1', mockPool);

        cache.resetDaily('key-1');

        assert.equal(cache.getDailySpend('key-1'), 0);
        assert.equal(cache.getMonthlySpend('key-1'), 50.0);
    });
    ...
    it('getForKey returns the combined cached spend values', async () => {
        ...
        const mockPool = createMockPool(2.25, 12.5);
        await cache.refresh('key-1', mockPool);
...
describe('checkBudget', () => {
    ...
    it('blocks when daily spend exceeds daily limit', async () => {
        ...
        const result = await checkBudget(
            { id: 'key-1', daily_budget_usd: 4.0, monthly_budget_usd: null },
            cache,
            mockPool
        );
        assert.equal(result.allowed, false);
        assert.equal(result.dailySpend, 5.0);
    });

    it('blocks when monthly spend exceeds monthly limit', async () => {
        let time = 1000;
        const cache = new SpendCache({ ttlMs: 60_000, now: () => time });
        const mockPool = createMockPool(1.0, 50.0);
        await cache.refresh('key-1', mockPool);

        const result = await checkBudget(
            { id: 'key-1', daily_budget_usd: null, monthly_budget_usd: 40.0 },
            cache,
            mockPool
        );
        assert.equal(result.allowed, false);
        assert.equal(result.monthlySpend, 50.0);
    });
    ...
    it('allows when spend is below all limits', async () => {
        ...
        const result = await checkBudget(
            { id: 'key-1', daily_budget_usd: 5.0, monthly_budget_usd: 100.0 },
            cache,
            mockPool
        );
        assert.equal(result.allowed, true);
        assert.equal(result.dailyLimit, 5.0);
        assert.equal(result.monthlyLimit, 100.0);
    });
    ...
    it('refreshes cache when stale', async () => {
        ...
        assert.equal(result.allowed, true);
        assert.equal(result.dailySpend, 1.0);
    });
});
...
describe('applyResponseFilters', () => {
    ...
    it('returns original text when text is empty', () => {
        assert.equal(
            applyResponseFilters('', [{ find: 'a', replace: 'b' }]),
            ''
        );
    });
...
describe('evaluateLoopSignal', () => {
    ...
    it('does not detect loop when responses are different', () => {
        ...
        const result = evaluateLoopSignal(
            state,
            { content: 'Response D' },
            settings
        );
