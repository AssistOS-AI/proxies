...
import { redactLogEntry, redactPayload } from '../../observability/redaction.mjs';
...
describe('BroadcastHub', () => {
  ...
  it('tracks subscriber count', () => {
    ...
    const mockStream = { onClose(fn) { this._cleanup = fn; }, send() {}, comment() {}, close() {} };
  ...
  it('filters by soul_id', () => {
    ...
    const mockStream = {
      onClose() {},
      send(event, data) { received.push(JSON.parse(data)); },
      comment() {},
      close() {},
    };
    ...
    // Matching log
    hub.publish({ soul_id: 'user-1', requested_model: 'gpt-4', status: 'succeeded' });
    // Non-matching log
    hub.publish({ soul_id: 'user-2', requested_model: 'gpt-4', status: 'succeeded' });
  ...
  it('filters by model', () => {
    ...
    hub.publish({ soul_id: 'u1', requested_model: 'claude-3', status: 'succeeded' });
  ...
  it('redacts payloads in normal stream', () => {
    ...
    assert.equal(received.length, 1);
    assert.equal(received[0].request_payload, undefined, 'should redact request_payload');
    assert.equal(received[0].response_payload, undefined, 'should redact response_payload');
    assert.equal(received[0].request_headers, undefined, 'should redact request_headers');
  });
  ...
  it('sends full payload on soul-specific stream', () => {
    ...
    assert.equal(received.length, 1);
    assert.ok(received[0].request_payload, 'should include request_payload');
    assert.ok(received[0].response_payload, 'should include response_payload');
  });
});
