...
describe('Migration files', () => {
  it('001-initial-schema.sql exists and is valid SQL', async () => {
    const path = new URL('../../db/migrations/001-initial-schema.sql', import.meta.url).pathname;
  ...
  it('all tables have created_at columns', async () => {
    ...
    const tablesWithCreatedAt = [
      'api_keys', 'providers', 'provider_accounts', 'models',
      'model_aliases', 'tiers', 'tier_models', 'middlewares',
      'middleware_assignments', 'blacklist_rules',
    ];

    for (const table of tablesWithCreatedAt) {
      const block = tableBlocks.find(b => b.trimStart().startsWith(table));
