...
import { toProviderView, toProviderList } from '../../management/provider-view.mjs';
import { toDiscoveryView, toDiscoveryList } from '../../management/model-discovery-view.mjs';
...
describe('management/provider-view', () => {
  ...
  it('prefers an explicit name when provider_key is absent', () => {
    const view = toProviderView({ id: 'p1', name: 'custom', provider_key: null });
  ...
  describe('toProviderList', () => {
    it('maps rows and skips falsy entries', () => {
      const list = toProviderList([
        ...
        null,
        { id: 'p2', provider_key: 'anthropic', display_name: 'Anthropic' },
      ]);
