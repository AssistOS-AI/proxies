...
import { encrypt, decrypt, ensureEncryptionKey } from '../../runtime/security/encryption.mjs';
...
import { loginAdmin, requireAdmin, createAdminSessionCookie } from '../../runtime/security/dashboard-auth.mjs';
import { extractBearerToken, hashApiKey, derivePepper } from '../../runtime/security/api-key-auth.mjs';
import {
...
describe('ensureEncryptionKey', () => {
  it('decodes a valid base64 ENCRYPTION_KEY', () => {
    ...
    const config = { ENCRYPTION_KEY: raw.toString('base64'), DATA_DIR: '/tmp' };
    const result = ensureEncryptionKey(config);
    assert.deepEqual(result, raw);
  });

  it('throws when ENCRYPTION_KEY decodes to wrong length', () => {
    const config = { ENCRYPTION_KEY: randomBytes(16).toString('base64'), DATA_DIR: '/tmp' };
...
describe('hashApiKey', () => {
  ...
  it('produces different hashes for different tokens', () => {
    const pepper = 'pepper';
    assert.notEqual(hashApiKey('token-a', pepper), hashApiKey('token-b', pepper));
  });

  it('produces different hashes for different peppers', () => {
    assert.notEqual(hashApiKey('token', 'pepper-a'), hashApiKey('token', 'pepper-b'));
  });
});

describe('derivePepper', () => {
  it('prefers API_KEY_HASH_PEPPER when set', () => {
    assert.equal(derivePepper({ API_KEY_HASH_PEPPER: 'p', ENCRYPTION_KEY: 'e' }), 'p');
  });

  it('falls back to ENCRYPTION_KEY', () => {
    assert.equal(derivePepper({ API_KEY_HASH_PEPPER: null, ENCRYPTION_KEY: 'e' }), 'e');
  });

  it('throws when neither is set', () => {
    assert.throws(
      () => derivePepper({ API_KEY_HASH_PEPPER: null, ENCRYPTION_KEY: null }),
      /neither/i,
    );
  });
});
...
describe('extractBearerToken', () => {
  ...
  it('throws AuthenticationRequiredError for null header', () => {
    assert.throws(() => extractBearerToken(null), (err) => {
  ...
  it('throws AuthenticationRequiredError for undefined header', () => {
    assert.throws(() => extractBearerToken(undefined), (err) => {
      assert(err instanceof AuthenticationRequiredError);
      return true;
    });
  });
  ...
  it('throws AuthenticationRequiredError for non-Bearer scheme', () => {
    assert.throws(() => extractBearerToken('Basic abc'), (err) => {
      assert(err instanceof AuthenticationRequiredError);
  ...
  it('throws AuthenticationRequiredError for empty token', () => {
    assert.throws(() => extractBearerToken('Bearer '), (err) => {
      assert(err instanceof AuthenticationRequiredError);
      return true;
    });
  });
...
describe('admin session sign/verify (loginAdmin + requireAdmin)', () => {
  ...
  it('requireAdmin accepts a valid cookie token', async () => {
    const { token } = await loginAdmin('s3cret', config);
    const req = { headers: { cookie: `soul_session=${encodeURIComponent(token)}` } };
    const decoded = requireAdmin(req, config);
    assert(decoded.exp > Date.now());
  });
...
describe('compareDashboardPassword', () => {
  ...
  it('returns false for different length passwords', () => {
    assert.equal(compareDashboardPassword('short', 'much-longer-password'), false);
  });
