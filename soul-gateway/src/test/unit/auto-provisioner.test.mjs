...
async function withStubbedModelsDao(stub, fn) {
  ...
  const mod = await import(`../../runtime/providers/auto-provisioner.mjs?mock=${Date.now()}${Math.random()}`);
  try {
    return await fn(mod);
  } finally {
    mocked.restore();
...
describe('auto-provisioner.autoProvisionModels', () => {
  ...
  it('looks up the plugin via provider.adapter_key (Bug A regression fuse)', async () => {
    ...
    assert.equal(log._entries.warn.filter((w) => w.msg.includes('plugin missing')).length, 0,
  ...
  it('warns with a structured message when the plugin has no discoverModels function', async () => {
    ...
    const warn = log._entries.warn.find((w) => w.msg.includes('plugin missing'));
    assert.ok(warn, 'expected a warn log when plugin is missing discoverModels');
  ...
  it('warns (not silent no-op) when no plugin is registered at all', async () => {
    ...
    const result = await autoProvisionModels(appCtx, {
      id: 'p1', provider_key: 'ghost', adapter_key: 'ghost',
    });
    assert.equal(result.discovered, 0);
    assert.ok(log._entries.warn.find((w) => w.msg.includes('plugin missing')));
  });

  it('leases credentials, passes them to plugin.discoverModels, and releases after', async () => {
    const lease = { accountId: 'acc-1', oauth: { accessToken: 'tok' }, authType: 'oauth' };
  ...
  it('releases the credential lease even when the plugin throws', async () => {
    ...
    assert.equal(result.discovered, 0);
    assert.equal(credentialManager._state.released, 1);
    assert.ok(log._entries.warn.find((w) => w.msg.includes('auto-provision discovery failed')));
  });

  it('inserts only new models, skipping ones that already exist', async () => {
    const discovered = [
      { modelId: 'gpt-5.2-codex', displayName: 'gpt-5.2-codex', contextWindow: 272000 },
      { modelId: 'gpt-5-codex', displayName: 'gpt-5-codex', contextWindow: 128000 },
      { modelId: 'gpt-5.1-codex', displayName: 'gpt-5.1-codex' },
    ];
    const plugin = { async discoverModels() { return discovered; } };

    const existing = new Set(['codex-api/gpt-5-codex']);
    const created = [];
    const stub = {
      findByKey: async (_pool, key) => (existing.has(key) ? { id: 'x', model_key: key } : null),
    ...
    const appCtx = createMockAppCtx({
      catalog: createMockCatalog({ 'codex-api': plugin }),
      credentialManager: createMockCredentialManager({ oauth: { accessToken: 'tok' } }),
      log,
    });

    const result = await withStubbedModelsDao(stub, (mod) => mod.autoProvisionModels(appCtx, {
      id: 'p1', provider_key: 'codex-api', adapter_key: 'codex-api',
    }));
    ...
    assert.deepEqual(
      created.map((c) => c.modelKey).sort(),
      ['codex-api/gpt-5.1-codex', 'codex-api/gpt-5.2-codex'],
    );
  ...
  it('tolerates duplicate-key races without surfacing them as warnings', async () => {
    ...
    assert.equal(log._entries.warn.filter((w) => w.msg.includes('model create failed')).length, 0);
  });

  it('keys inserted models as `${provider_key}/${modelId}` to match dashboard convention', async () => {
    const plugin = {
      async discoverModels() { return [{ modelId: 'm1' }, { modelId: 'm2' }]; },
    };
    const captured = [];
    const stub = {
      findByKey: async () => null,
      create: async (_pool, row) => { captured.push(row.modelKey); return { id: 'x' }; },
    };
    const appCtx = createMockAppCtx({
    ...
    await withStubbedModelsDao(stub, (mod) => mod.autoProvisionModels(appCtx, {
      id: 'p1', provider_key: 'my-custom-codex', adapter_key: 'my-adapter',
    }));
    assert.deepEqual(captured, ['my-custom-codex/m1', 'my-custom-codex/m2']);
  });
  ...
  it('returns early (and does not throw) when the catalog is not installed', async () => {
    ...
    const result = await autoProvisionModels(appCtx, {
      id: 'p1', provider_key: 'x', adapter_key: 'x',
    });
    assert.equal(result.discovered, 0);
    assert.equal(result.created, 0);
  });
