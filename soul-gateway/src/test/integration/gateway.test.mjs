...
describe('Dashboard Auth', () => {
  it('POST /management/auth/login succeeds with correct password', async () => {
    const { status, data } = await api('POST', '/management/auth/login', { password: DASHBOARD_PASSWORD });
    assert.equal(status, 200);
    assert.equal(data.ok, true);
  ...
  it('POST /management/auth/login fails with wrong password', async () => {
    const { status } = await api('POST', '/management/auth/login', { password: 'wrong' });
...
describe('Management — Providers', () => {
  ...
  it('GET /management/providers/templates returns templates', async () => {
    const { status, data } = await api('GET', '/management/providers/templates');
    assert.equal(status, 200);
    assert.ok(data);
  });
});
...
describe('Management — Tiers', () => {
  it('GET /management/tiers returns tiers', async () => {
    const { status, data } = await api('GET', '/management/tiers');
    assert.equal(status, 200);
    const tiers = data.data || data;
    assert.ok(Array.isArray(tiers));
    // We imported tiers from production
    assert.ok(tiers.length > 0, 'should have imported tiers');
  });

  it('tiers have model membership', async () => {
    const { data } = await api('GET', '/management/tiers');
    const tiers = data.data || data;
    const fastTier = tiers.find(t => (t.tier_key || t.name || '').includes('fast'));
    if (fastTier) {
      assert.ok(fastTier.models || fastTier.model_refs, 'tier should have models');
    }
  });
});
...
describe('Management — Logs', () => {
  it('GET /management/logs returns paginated logs', async () => {
    ...
    assert.equal(status, 200);
    assert.ok(data.data !== undefined || data.rows !== undefined || Array.isArray(data));
  });
});
...
describe('Public API — /v1/chat/completions', () => {
  ...
  it('POST with unknown model returns 404', async () => {
    const { status, data } = await clientApi('POST', '/v1/chat/completions', {
  ...
  it('POST with valid tier reaches execution stage', async () => {
    const { status, data } = await clientApi('POST', '/v1/chat/completions', {
      model: 'axl/fast',
      messages: [{ role: 'user', content: 'Say hello' }],
    });
    // 200 = full success, 429/502/503/504 = provider error (routing worked)
    // 500 = execution engine issue (acceptable for now — providers not connected)
    assert.ok(
      [200, 429, 500, 502, 503, 504].includes(status),
