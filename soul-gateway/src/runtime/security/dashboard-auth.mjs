...
export async function loginAdmin(password, config) {
  if (!config.DASHBOARD_PASSWORD) {
    throw new AuthenticationRequiredError('Dashboard password not configured');
  }
...
function resolveSigningKey(config) {
  if (config.ADMIN_SESSION_SIGNING_KEY) return config.ADMIN_SESSION_SIGNING_KEY;
  if (config.ENCRYPTION_KEY) {
    return createHmac('sha256', config.ENCRYPTION_KEY).update('admin-session').digest('hex');
  }
