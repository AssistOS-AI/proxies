...
export async function collectNormalizedStream(stream, options = {}) {
        ...
        switch (chunk.type) {
            ...
            case 'tool_call_delta':
                ...
                if (payload.arguments) {
                    toolCalls[payload.index].function.arguments +=
                        payload.arguments;
                }
                if (payload.name) {
            ...
            case 'done':
                finishReason =
                    payload.finish_reason || chunk.finish_reason || 'stop';
                if (payload.rawResponse || chunk.rawResponse)
                    rawResponse = payload.rawResponse || chunk.rawResponse;
                if (payload.responseMeta || chunk.responseMeta)
                    responseMeta = payload.responseMeta || chunk.responseMeta;
                break;
            ...
            case 'error':
                throw (
                    ...
                    chunk.error ||
                    new Error(
                        payload.message || chunk.message || 'stream error'
                    )
                );
        }
    }
    ...
    const excerpt =
        fullText.length > maxExcerptChars
