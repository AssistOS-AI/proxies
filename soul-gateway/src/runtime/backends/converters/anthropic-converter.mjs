...
export function toProviderRequest(normalizedReq, modelRecord, providerRecord) {
    const body = {
        ...
        max_tokens:
            normalizedReq.max_tokens ?? modelRecord.default_max_tokens ?? 4096,
    };
    ...
    if (normalizedReq.stop != null)
        body.stop_sequences = Array.isArray(normalizedReq.stop)
            ? normalizedReq.stop
            : [normalizedReq.stop];
    ...
    switch (type) {
        ...
        case 'content_block_start': {
            state.currentBlockIndex =
                rawChunk.index ?? state.currentBlockIndex + 1;
            ...
            if (rawChunk.usage) {
                chunks.push({
                    ...
                    data: {
                        ...
                        total_tokens:
                            (rawChunk.usage.input_tokens || 0) +
                            (rawChunk.usage.output_tokens || 0),
                    },
                });
            }
            break;
        ...
        case 'error': {
            chunks.push({
                ...
                data: {
                    message:
                        rawChunk.error?.message || 'Unknown Anthropic error',
