    ...
    switch (chunk?.type) {
      case 'message_start': {
        ...
        yield {
          type: 'message_start',
          data: {
            id: chunk.id || chunk.data?.id || meta.requestId || null,
            model: chunk.model || chunk.data?.model || meta.model || null,
      ...
      case 'done': {
        ...
        yield {
          type: 'done',
          data: {
            finish_reason: chunk.stopReason || chunk.finish_reason || chunk.data?.finish_reason || 'stop',
            model: meta.model || chunk.model || chunk.data?.model || null,
          },
        };
        break;
      }
      ...
      case 'error': {
        yield {
          type: 'error',
          error: chunk.error || new Error(chunk.message || chunk.data?.message || 'Provider stream error'),
        };
        break;
      }
