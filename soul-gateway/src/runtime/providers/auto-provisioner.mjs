 ...
 *
 * The plugin is looked up by the provider's own `adapter_key`, which
 ...
 */
export async function autoProvisionModels(appCtx, provider, oauthAdapterKey = null) {
  ...
  const pluginKey = provider.adapter_key || oauthAdapterKey;
  const plugin = catalog.getPlugin(pluginKey);
  if (!plugin || typeof plugin.discoverModels !== 'function') {
    log.warn('auto-provision skipped: plugin missing or has no discoverModels', {
  ...
  try {
    ...
    if (credentialLease && credentialManager) credentialManager.release(credentialLease);
  }
  ...
  if (!discovered.length) {
    log.info('auto-provision returned no models', { provider: provider.provider_key, pluginKey });
    return { discovered: 0, created: 0 };
  }
  ...
  for (const model of discovered) {
    const providerModelId = model.modelId || model.model_key || model.id;
    if (!providerModelId) continue;
    const modelKey = `${provider.provider_key}/${providerModelId}`;
    ...
    try {
      await modelsDao.create(appCtx.pool, {
        modelKey,
        displayName: model.displayName || model.display_name || providerModelId,
  ...
  if (created > 0) {
    ...
    requestRuntimeRefresh(appCtx, { snapshot: true, reason: 'auto-provision' });
  }
...
/**
 * @deprecated Use `autoProvisionModels`. This alias is kept so the
 * OAuthManager call site doesn't need to change in the same commit.
 */
export const autoProvisionAfterOAuth = autoProvisionModels;
