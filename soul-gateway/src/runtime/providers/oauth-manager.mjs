...
export class OAuthManager {
    ...
    registerAdapter(adapter) {
        ...
        this._log.info('oauth_adapter_registered', {
            key: adapter.key,
    ...
    async startAuthFlow(providerId, adapterKey, options = {}) {
        ...
        if (!adapter) {
            throw new Error(
                `No OAuth adapter registered for key: ${adapterKey}`
            );
        }
    ...
    async completeAuthFlow(providerId, params = {}) {
        const flow = params.flowId
            ? this._activeFlows.get(params.flowId)
            : null;
        ...
        if (this._oauthCredentialStore) {
            await this._oauthCredentialStore.write(
                credentialsPath,
        ...
        if (this._appCtx) {
            try {
                const { autoProvisionModels } = await import(
                    './auto-provisioner.mjs'
                );
                const providersDao = await import(
                    '../../db/dao/providers-dao.mjs'
                );
                const providerRow = await providersDao.findById(
                    this._pool,
                    providerId
                );
                if (providerRow) {
    ...
    /**
     * Start an OAuth flow from a provider record returned by the
     * management layer.
     */
    async startFlow(providerRecord, options = {}) {
        ...
        if (!provider?.id) {
            throw new Error(
                'providerRecord.id is required to start an OAuth flow'
            );
        }
    ...
    /**
     * Complete an auth-code flow from callback query params.
     */
    ...
    async revoke(accountId, adapterKey) {
        ...
        try {
            ...
            this._log.error('oauth_revoke_failed', {
                accountId,
                error: err.message,
            });
        }
    ...
    async _doRefresh(accountId, adapterKey) {
        ...
        if (!adapter?.refreshTokens) {
            throw new Error(
                `OAuth adapter ${adapterKey} does not support token refresh`
            );
        }
        ...
        try {
            ...
            const newTokens = await adapter.refreshTokens({
                accessToken:
                    storedTokens.accessToken || account.metadata?.access_token,
                refreshToken:
         ...
         WHERE id = $1 AND deleted_at IS NULL`,
            ...
            await this._accountPool.markActive(accountId, {
                accessTokenExpiresAt:
                    refreshedPayload.accessTokenExpiresAt || null,
                refreshTokenExpiresAt:
                    refreshedPayload.refreshTokenExpiresAt || null,
            });

            this._log.info('oauth_tokens_refreshed', { accountId, adapterKey });
        } catch (err) {
            await this._accountPool.markErrored(
                accountId,
                'refresh_failed',
                err.message
            );
            this._log.error('oauth_refresh_failed', {
                accountId,
                adapterKey,
                error: err.message,
            });
    ...
    async _allocateCredentialsPath(
        providerId,
        externalAccountId,
        fallbackName
    ) {
        if (!this._oauthCredentialStore) return null;
        return this._oauthCredentialStore.allocatePath(
            providerId,
            externalAccountId,
            fallbackName
        );
    ...
    async _readStoredCredentials(account) {
        ...
        try {
            return (
                (await this._oauthCredentialStore.read(
                    account.credentials_path
                )) || {}
            );
        } catch {
