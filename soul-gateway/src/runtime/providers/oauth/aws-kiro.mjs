import { buildPkceAuthUrl, computeExpiryIso, decodeJwtPayload, exchangeAuthorizationCode, generatePkceVerifier } from './common.mjs';
...
export const oauthAdapter = {
  ...
  async handleCallback(ctx) {
    ...
    const idPayload = decodeJwtPayload(tokenData.idToken || tokenData.id_token);
    const email = tokenData.email || idPayload?.email || null;
    const externalAccountId = tokenData.profileArn || idPayload?.sub || email;
    ...
    return {
      ...
      accessTokenExpiresAt: computeExpiryIso(tokenData.expiresIn || tokenData.expires_in || 3600),
  ...
  async refreshTokens(tokens) {
    ...
    if (!response.ok) {
      const error = new Error(data.message || `Kiro refresh failed (${response.status})`);
    ...
    return {
      ...
      accessTokenExpiresAt: computeExpiryIso(data.expiresIn || data.expires_in || 3600),
