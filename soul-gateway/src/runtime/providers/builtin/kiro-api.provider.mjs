...
// Known Kiro models
const KNOWN_MODELS = [
  { modelId: 'claude-sonnet-4', displayName: 'Claude Sonnet 4 (Kiro)', contextWindow: 200000, maxOutputTokens: 16384, supportsTools: true, supportsStreaming: true, supportsVision: true },
  { modelId: 'claude-3.7-sonnet', displayName: 'Claude 3.7 Sonnet (Kiro)', contextWindow: 200000, maxOutputTokens: 16384, supportsTools: true, supportsStreaming: true, supportsVision: true },
  { modelId: 'claude-haiku-4.5', displayName: 'Claude Haiku 4.5 (Kiro)', contextWindow: 200000, maxOutputTokens: 8192, supportsTools: true, supportsStreaming: true, supportsVision: false },
  { modelId: 'claude-sonnet-4.5', displayName: 'Claude Sonnet 4.5 (Kiro)', contextWindow: 200000, maxOutputTokens: 16384, supportsTools: true, supportsStreaming: true, supportsVision: true },
  { modelId: 'deepseek-3.2', displayName: 'DeepSeek 3.2 (Kiro)', contextWindow: 128000, maxOutputTokens: 8192, supportsTools: true, supportsStreaming: true, supportsVision: false },
  { modelId: 'minimax-m2.1', displayName: 'MiniMax M2.1 (Kiro)', contextWindow: 128000, maxOutputTokens: 8192, supportsTools: false, supportsStreaming: true, supportsVision: false },
  { modelId: 'qwen3-coder-next', displayName: 'Qwen3 Coder Next (Kiro)', contextWindow: 128000, maxOutputTokens: 8192, supportsTools: true, supportsStreaming: true, supportsVision: false },
  { modelId: 'auto-kiro', displayName: 'Auto Kiro', contextWindow: 200000, maxOutputTokens: 16384, supportsTools: true, supportsStreaming: true, supportsVision: false },
];
...
export const providerPlugin = {
  ...
  validateProviderRecord(providerRecord) {
    // base_url should point to the Kiro API endpoint
  },

  validateModelRecord(modelRecord) {
    if (!modelRecord.provider_model_id && !modelRecord.model_key) {
      throw new Error('Kiro model requires provider_model_id or model_key');
    }
  },
  ...
  async testConnection(ctx) {
    ...
    if (!token) return { ok: false, detail: 'No Kiro credentials configured' };
  ...
  async execute(ctx) {
    ...
    const baseUrl = providerRecord.base_url || 'https://api.kiro.dev';
    const token = getCredentialToken(credentialLease);
    const headers = {
      Accept: 'application/vnd.amazon.eventstream',
    };
    const params = { stream: true };
    if (normalizedReq.max_tokens != null) params.max_tokens = normalizedReq.max_tokens;
    ...
    if (awsAccessKey && awsSecretKey) {
      ...
      const sigHeaders = signAWSRequest({
        ...
        body: JSON.stringify(kiroConverter.toProviderRequest(normalizedReq, resolvedModel, providerRecord)),
  ...
  classifyError(error, _ctx) {
    ...
    // AWS error codes
    if (errorCode === 'UnrecognizedClientException' || errorCode === 'AccessDeniedException' || status === HTTP_STATUS.UNAUTHORIZED || status === HTTP_STATUS.FORBIDDEN) {
      return new ProviderAuthError('kiro', message || 'Auth failed');
    }
    if (errorCode === 'ResourceNotFoundException' || status === HTTP_STATUS.NOT_FOUND) {
      return new ProviderModelNotFoundError('kiro', message || 'unknown');
    }
    if (errorCode === 'ThrottlingException' || errorCode === 'TooManyRequestsException' || status === HTTP_STATUS.TOO_MANY_REQUESTS) {
      if (looksLikeQuotaError(message)) {
        return new ProviderQuotaError('kiro');
      }
      return new ProviderRateLimitError('kiro');
    }
    if (errorCode === 'ValidationException' || status === HTTP_STATUS.BAD_REQUEST) {
      if (looksLikeContentPolicyError(message)) {
        return new ProviderContentPolicyError('kiro');
      }
    }
    if (errorCode === 'ServiceUnavailableException' || status === HTTP_STATUS.SERVICE_UNAVAILABLE) {
...
async function* makeKiroStream(url, headers, payload, signal) {
  ...
  if (contentType.includes('application/vnd.amazon.eventstream')) {
    ...
    for await (const frame of parseBinaryEventStream(response)) {
      const normalized = kiroConverter.fromProviderChunk(frame, converterState);
    ...
    for await (const event of parseSSE(response)) {
      ...
      const normalized = kiroConverter.fromProviderChunk(parsed, converterState);
      for (const chunk of normalized) {
        yield chunk;
      }
    }
...
function signAWSRequest({ method, url, headers, body, accessKey, secretKey, sessionToken, region, service, now }) {
