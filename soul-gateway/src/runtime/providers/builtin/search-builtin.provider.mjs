...
const manifest = {
    ...
    // correct baseUrl. Hide the raw `search-builtin` key from the
    // dropdown — picking it directly would leave the user with a blank
    // baseUrl and nothing meaningful to point at.
    hidden: true,
};
...
// entry is the hostname produced by the matching preset's baseUrl
...
export const providerPlugin = {
    ...
    validateModelRecord(modelRecord) {
        const engineId = modelRecord.providerModelId || modelRecord.modelKey;
    ...
    async testConnection(ctx) {
        ...
        if (!engineKey) {
            return secret
                ? {
                      ok: true,
                      detail: 'Search credentials present (engine unknown)',
                  }
        ...
        if (
            ...
            return {
                ok: true,
                detail: `${engine.displayName} does not require authentication`,
            };
        }
        ...
        if (!secret && engineKey !== 'duckduckgo' && engineKey !== 'searxng') {
            return {
                ok: false,
                detail: `${engine.displayName}: no API key configured`,
            };
        }

        return {
            ok: true,
            detail: `${engine.displayName} credentials present`,
        };
    },
    ...
    async execute(ctx) {
        ...
        const engineId =
            resolvedModel.providerModelId || resolvedModel.modelKey;
        const engineKey = engineId.replace(/^search-/, '');
        ...
        // Execute the search
        const rawResponse = await doSearchRequest(
            urlStr,
            method,
            headers,
            body
 ...
 *   1. ctx.resolvedModel.providerModelId — the execution path
 ...
 *   3. providerRecord.providerKey — matches the preset key for
 *      anything added via the Add Provider dropdown
 *      (providerKey === 'exa', 'brave', 'tavily', …).
 *   4. providerRecord.baseUrl hostname — last resort, covers
...
function resolveEngineKey(providerRecord, ctx = {}) {
    ...
    const resolvedRaw = ctx.resolvedModel?.providerModelId;
    if (resolvedRaw) {
    ...
    // 3. providerKey matches a preset key directly.
    ...
    if (baseUrl) {
        try {
            ...
            // Malformed baseUrl — fall through to null.
        }
    }
...
async function* emptyResultStream(requestId) {
    yield {
        type: 'message_start',
        data: { id: requestId, model: 'search', role: 'assistant' },
    };
        ...
        const req = reqFn(opts, (res) => {
            ...
            res.on('end', () => {
                ...
                if (res.statusCode >= 400) {
                    const err = new Error(
                        `Search API error: ${res.statusCode}`
                    );
...
async function executeDeepResearch(ctx, query, secret, settings) {
    ...
    const enginePromises = configuredEngines.map(async (engineKey) => {
        ...
        try {
            ...
            if (engine.buildUrl)
                urlStr = engine.buildUrl(query, { maxResults: 5 });
    ...
    const formatted = searchConverter.formatDeepResearchResults(
        topResults,
        query,
        engineCount
    );
    ...
    async function* stream() {
        ...
        return {
            ...
            rawResponse: null,
            responseMeta: { engines: engineCount, results: topResults.length },
        };
    }
