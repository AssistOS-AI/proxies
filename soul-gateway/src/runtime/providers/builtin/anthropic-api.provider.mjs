/**
 * Anthropic Messages API provider plugin.
 *
...
const KNOWN_MODELS = [
  { modelId: 'claude-sonnet-4-20250514', displayName: 'Claude Sonnet 4', contextWindow: 200000, maxOutputTokens: 16384, supportsTools: true, supportsStreaming: true, supportsVision: true },
  { modelId: 'claude-opus-4-20250514', displayName: 'Claude Opus 4', contextWindow: 200000, maxOutputTokens: 32768, supportsTools: true, supportsStreaming: true, supportsVision: true },
  { modelId: 'claude-3-5-sonnet-20241022', displayName: 'Claude 3.5 Sonnet', contextWindow: 200000, maxOutputTokens: 8192, supportsTools: true, supportsStreaming: true, supportsVision: true },
  { modelId: 'claude-3-5-haiku-20241022', displayName: 'Claude 3.5 Haiku', contextWindow: 200000, maxOutputTokens: 8192, supportsTools: true, supportsStreaming: true, supportsVision: false },
  { modelId: 'claude-3-haiku-20240307', displayName: 'Claude 3 Haiku', contextWindow: 200000, maxOutputTokens: 4096, supportsTools: true, supportsStreaming: true, supportsVision: true },
];
...
export const providerPlugin = {
  ...
  validateModelRecord(modelRecord) {
    if (!modelRecord.provider_model_id && !modelRecord.model_key) {
      throw new Error('Anthropic model requires provider_model_id or model_key');
    }
  },
  ...
  async testConnection(ctx) {
    const baseUrl = ctx.providerRecord?.base_url || 'https://api.anthropic.com';
    const headers = buildAnthropicHeaders(ctx.credentialLease, ctx.providerRecord?.settings, DEFAULT_API_VERSION);
    if (!headers) return { ok: false, detail: 'No credentials configured' };

    try {
      // Send a minimal request to check auth
      const apiVersion = ctx.providerRecord?.settings?.anthropic_version || DEFAULT_API_VERSION;
      ...
      const response = await doRequest(url, 'POST', {
      ...
      return { ok: false, detail: `HTTP ${response.statusCode}: ${resBody.slice(0, 200)}` };
    } catch (err) {
  ...
  async execute(ctx) {
    ...
    const baseUrl = providerRecord.base_url || 'https://api.anthropic.com';
    const apiVersion = providerRecord.settings?.anthropic_version || DEFAULT_API_VERSION;
    const authHeaders = buildAnthropicHeaders(credentialLease, providerRecord.settings, apiVersion);
    if (!authHeaders) {
    ...
    const params = {
      stream: true,
      max_tokens: normalizedReq.max_tokens ?? resolvedModel.default_max_tokens ?? 4096,
    };
    ...
    if (normalizedReq.stop != null) {
      params.stop_sequences = Array.isArray(normalizedReq.stop) ? normalizedReq.stop : [normalizedReq.stop];
    }
    ...
    return createAchillesExecutionHandle(ctx, achillesAnthropic, {
      ...
      apiKey: authHeaders['x-api-key'] || getCredentialToken(credentialLease),
      baseURL: baseUrl,
      signal,
      params,
      headers: authHeaders,
  ...
  classifyError(error, _ctx) {
    ...
    if (errorType === 'authentication_error' || status === HTTP_STATUS.UNAUTHORIZED) {
      return new ProviderAuthError('anthropic', body.error?.message || 'Authentication failed');
    }
    if (errorType === 'permission_error' || status === HTTP_STATUS.FORBIDDEN) {
      return new ProviderAuthError('anthropic', 'Permission denied');
    }
    if (errorType === 'not_found_error' || status === HTTP_STATUS.NOT_FOUND) {
      return new ProviderModelNotFoundError('anthropic', body.error?.message || 'unknown');
    }
    if (errorType === 'rate_limit_error' || status === HTTP_STATUS.TOO_MANY_REQUESTS) {
    ...
    if (errorType === 'invalid_request_error' || status === HTTP_STATUS.BAD_REQUEST) {
...
function buildAnthropicHeaders(credentialLease, settings = {}, apiVersion = DEFAULT_API_VERSION) {
...
async function* makeAnthropicStream(url, headers, payload, signal) {
  ...
  for await (const event of parseSSE(response)) {
    ...
    const normalized = anthropicConverter.fromProviderChunk(parsed, converterState);
