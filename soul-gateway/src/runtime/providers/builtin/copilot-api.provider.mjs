...
const KNOWN_MODELS = [
  ...
  { modelId: 'gpt-4o-mini', displayName: 'GPT-4o Mini', contextWindow: 128000, maxOutputTokens: 16384, supportsTools: true, supportsStreaming: true, supportsVision: true },
  ...
  { modelId: 'gpt-4.1-mini', displayName: 'GPT-4.1 Mini', contextWindow: 1000000, maxOutputTokens: 32768, supportsTools: true, supportsStreaming: true, supportsVision: true },
  { modelId: 'gpt-4.1-nano', displayName: 'GPT-4.1 Nano', contextWindow: 1000000, maxOutputTokens: 32768, supportsTools: true, supportsStreaming: true, supportsVision: false },
  { modelId: 'claude-sonnet-4', displayName: 'Claude Sonnet 4', contextWindow: 200000, maxOutputTokens: 16384, supportsTools: true, supportsStreaming: true, supportsVision: true },
  { modelId: 'claude-3.5-sonnet', displayName: 'Claude 3.5 Sonnet', contextWindow: 200000, maxOutputTokens: 8192, supportsTools: true, supportsStreaming: true, supportsVision: true },
  { modelId: 'o1-preview', displayName: 'o1-preview', contextWindow: 128000, maxOutputTokens: 32768, supportsTools: false, supportsStreaming: true, supportsVision: false },
  { modelId: 'o1-mini', displayName: 'o1-mini', contextWindow: 128000, maxOutputTokens: 65536, supportsTools: false, supportsStreaming: true, supportsVision: false },
  { modelId: 'o3-mini', displayName: 'o3-mini', contextWindow: 200000, maxOutputTokens: 100000, supportsTools: true, supportsStreaming: true, supportsVision: false },
];
...
export const providerPlugin = {
  ...
  validateModelRecord(modelRecord) {
    if (!modelRecord.provider_model_id && !modelRecord.model_key) {
      throw new Error('Copilot model requires provider_model_id or model_key');
    }
  },

  async discoverModels(ctx) {
    const baseUrl = ctx?.providerRecord?.base_url || 'https://api.githubcopilot.com';
    ...
    try {
      ...
      return (parsed.data || []).map((m) => ({
        ...
        contextWindow: m.capabilities?.limits?.max_prompt_tokens || null,
        maxOutputTokens: m.capabilities?.limits?.max_output_tokens || null,
  ...
  async execute(ctx) {
    ...
    const baseUrl = providerRecord.base_url || 'https://api.githubcopilot.com';
    ...
    if (normalizedReq.tool_choice != null) params.tool_choice = normalizedReq.tool_choice;
  ...
  classifyError(error, _ctx) {
    ...
    if (status === HTTP_STATUS.UNAUTHORIZED || status === HTTP_STATUS.FORBIDDEN) {
      return new ProviderAuthError('copilot', message || 'Copilot auth failed');
    }
    if (status === HTTP_STATUS.NOT_FOUND) {
      return new ProviderModelNotFoundError('copilot', message || 'unknown');
    }
  ...
  for await (const event of parseSSE(response)) {
    if (event.data === '[DONE]') {
      ...
      if (endpoint === 'completions') {
        const lastChunks = copilotConverter.fromProviderChunk('[DONE]', converterState, endpoint);
    ...
    const normalized = copilotConverter.fromProviderChunk(parsed, converterState, endpoint);
