...
export const providerPlugin = {
  ...
  validateProviderRecord(providerRecord) {
    if (!providerRecord.base_url) {
      throw new Error('OpenAI provider requires a base_url');
    }
  },

  validateModelRecord(modelRecord) {
    if (!modelRecord.provider_model_id && !modelRecord.model_key) {
      throw new Error('OpenAI model requires provider_model_id or model_key');
    }
  },
  ...
  async execute(ctx) {
    const { request: normalizedReq, resolvedModel, providerRecord, credentialLease, signal, logger } = ctx;
    const baseUrl = providerRecord.base_url || 'https://api.openai.com/v1';
    const modelId = resolvedModel.provider_model_id || resolvedModel.model_key;
    ...
    if (normalizedReq.response_format != null) params.response_format = normalizedReq.response_format;
    if (settings.extra_body) Object.assign(params, settings.extra_body);

    const headers = {};
    // OpenRouter-specific headers
    if (settings.openrouter_referer) headers['HTTP-Referer'] = settings.openrouter_referer;
    if (settings.openrouter_title) headers['X-Title'] = settings.openrouter_title;
    if (settings.extra_headers) Object.assign(headers, settings.extra_headers);
  ...
  classifyError(error, _ctx) {
    ...
    if (status === HTTP_STATUS.NOT_FOUND) {
      const model = body.error?.param === 'model' ? body.error?.message : 'unknown';
      return new ProviderModelNotFoundError('openai', model);
    }
    if (status === HTTP_STATUS.TOO_MANY_REQUESTS) {
      if (errorType === 'insufficient_quota' || errorType === 'billing_hard_limit_reached') {
...
async function* makeSSEStream(url, headers, payload, signal, logger) {
  ...
  for await (const event of parseSSE(response)) {
    if (event.data === '[DONE]') {
      yield { type: 'done', data: { finish_reason: state.lastFinishReason || 'stop', model: state.model } };
      return;
    }
