...
export class AccountPool {
  ...
  async getNextAccount(providerId, options = {}) {
    ...
    const accounts = await this._accountsDao.listByProvider(this._pool, providerId);

    // Filter to eligible candidates
    const candidates = accounts.filter((acc) => {
      // Must be active or refreshing
      if (acc.status !== 'active' && acc.status !== 'refreshing') return false;
      ...
      if (exhaustion) {
        if (!exhaustion.resetTime || exhaustion.resetTime > now) return false;
        // Expired exhaustion — clean up
        this._exhausted.delete(acc.id);
      }

      // Exclude DB-level quota exhausted accounts
      if (acc.quota_resets_at && new Date(acc.quota_resets_at) > now) return false;
  ...
  async markExhausted(providerId, accountId, resetTime = null) {
    ...
    try {
      await this._accountsDao.markExhausted(this._pool, accountId, resetTime);
    } catch (err) {
    ...
    this._log.info('account_exhausted', { providerId, accountId, resetTime });
  }
  ...
  async markActive(accountId, tokenInfo = {}) {
    ...
    try {
      await this._accountsDao.updateTokenExpiry(this._pool, accountId, tokenInfo);
    } catch (err) {
  ...
  async markErrored(accountId, errorType, errorMessage) {
    try {
      await this._accountsDao.updateStatus(this._pool, accountId, 'reauth_required', {
        lastErrorType: errorType,
        lastErrorMessage: errorMessage,
      });
    } catch (err) {
      this._log.error('account_mark_errored_failed', { accountId, error: err.message });
    }
  }
