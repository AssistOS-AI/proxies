...
export class ProviderCatalog {
     ...
     * `text NOT NULL`, so callers use the canonical transport key directly.
     *
            ...
            for (const [key, plugin] of old.plugins) {
                if (typeof plugin.shutdown === 'function') {
                    plugin.shutdown().catch((err) => {
                        this._log.error('old_plugin_shutdown_failed', {
                            key,
                            error: err.message,
                        });
                    });
    ...
    getTemplates() {
        ...
        for (const [key, plugin] of this._plugins) {
            ...
            templates[key] = {
                ...
                auth_type:
                    plugin.manifest.authStrategy === 'oauth'
                        ? 'managed'
                        : 'api_key',
                ...
                supported_formats: plugin.manifest.supportedFormats || [
                    'openai_chat',
                ],
            };
        }
    ...
    async testConnection(providerRecord, options = {}) {
        ...
        if (!target || typeof target.testConnection !== 'function') {
            return {
                ok: false,
                detail: 'Provider plugin not loaded or does not support testConnection',
            };
        }

        const providerCtx = await this._buildProviderLifecycleContext(
            providerRecord,
            options
        );
        try {
    ...
    async shutdownAll() {
        ...
        for (const [key, plugin] of this._plugins) {
            if (typeof plugin.shutdown === 'function') {
                shutdowns.push(
                    plugin.shutdown().catch((err) => {
                        this._log.error('plugin_shutdown_failed', {
                            key,
                            error: err.message,
                        });
                    })
    ...
    _resolveLifecycleTarget(providerRecord, options = {}) {
        ...
        if (
            normalizedProvider.providerMode === 'custom' &&
            options.transportCatalog
        ) {
            return options.transportCatalog.getTransport(
                normalizedProvider.adapterKey || key
            );
        }
