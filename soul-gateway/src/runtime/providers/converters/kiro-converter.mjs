...
export function fromProviderChunk(rawChunk, state) {
  ...
  switch (eventType) {
    ...
    case 'message_stop': {
      ...
      chunks.push({
        type: 'done',
        data: { finish_reason: mapKiroStopReason(reason), model: state.model },
      });
      break;
    }
      ...
      if (usage) {
        chunks.push({
          ...
          data: {
            ...
            total_tokens: (usage.inputTokens || usage.input_tokens || 0) +
    ...
    case 'exception': {
      chunks.push({
        type: 'error',
        data: {
          message: event.message || event.error?.message || 'Kiro error',
...
function mapRoleToKiro(role) {
  switch (role) {
    ...
    case 'tool': return 'user'; // tool results sent as user turns in Kiro
    default: return 'user';
  }
}
