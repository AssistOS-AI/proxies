 ...
 */
export function toCompletionsRequest(normalizedReq, modelRecord, _providerRecord) {
  const body = {
  ...
  if (normalizedReq.max_tokens != null) body.max_tokens = normalizedReq.max_tokens;
  ...
  if (normalizedReq.tools && normalizedReq.tools.length > 0) body.tools = normalizedReq.tools;
  if (normalizedReq.tool_choice != null) body.tool_choice = normalizedReq.tool_choice;
 ...
 */
export function toResponsesRequest(normalizedReq, modelRecord, _providerRecord) {
  const model = modelRecord.provider_model_id || modelRecord.model_key;
  ...
  if (normalizedReq.max_tokens != null) body.max_output_tokens = normalizedReq.max_tokens;
  if (normalizedReq.temperature != null) body.temperature = normalizedReq.temperature;
  if (normalizedReq.top_p != null) body.top_p = normalizedReq.top_p;
...
export function toProviderRequest(normalizedReq, modelRecord, providerRecord) {
  const modelId = modelRecord.provider_model_id || modelRecord.model_key;
  const endpoint = resolveEndpoint(modelId, providerRecord);

  if (endpoint === 'responses') {
    return {
      endpoint,
      body: toResponsesRequest(normalizedReq, modelRecord, providerRecord),
...
export function fromCompletionsChunk(rawChunk, state) {
  ...
  if (rawChunk === '[DONE]' || rawChunk?.done) {
    chunks.push({ type: 'done', data: { finish_reason: state.lastFinishReason || 'stop', model: state.model } });
    return chunks;
  }
  ...
  switch (eventType) {
    ...
    case 'response.in_progress': {
      ...
      if (state.firstChunk) {
        ...
        chunks.push({
          type: 'message_start',
          data: { id: resp.id, model: state.model, role: 'assistant' },
        });
      }
      break;
    }
    ...
    case 'response.completed': {
      ...
      const reason = resp.status === 'completed' ? 'stop' : (resp.status || 'stop');
      chunks.push({
        type: 'done',
    ...
    case 'error': {
      chunks.push({
        type: 'error',
        data: {
          message: rawChunk.error?.message || rawChunk.message || 'Copilot Responses API error',
