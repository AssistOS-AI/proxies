...
| `FREE_MODELS_ENABLED` | `true` | Installs the free OpenRouter provider, baseline free models, and compatibility tiers once, on the first start. `false` skips the install; setting it back to `true` later installs the defaults on the next start. The install runs on any database without its marker, so set `false` before upgrading an existing gateway that should not receive the free defaults. |
...
| `LLM_DEFAULT_TIERS` | `fast,code,plan,write,deep,ultra,web-assist` | Comma-separated compatibility tier names created on first start. |
