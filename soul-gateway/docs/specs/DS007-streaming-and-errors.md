...
One reply without content is not empty: a reply that the token limit cut off before any text or tool call, which ends with finish reason <code>length</code> (typically a model that spent its budget reasoning), is returned as a completion with that finish reason, because the finish reason tells the caller to raise its token limit. A cascade child whose retry policy sets <code>lengthWithoutContentFails</code> treats that reply as an empty response instead, so the next child is tried. The free-model execution policy sets it (DS002), because in a cascade the gateway, not the caller, chose the model, and the next child may answer within the caller's limit. A direct request always receives the length finish, because no other model would be tried.
