...
A stream that reaches <code>done</code> or its end without any text or tool-call delta is an empty response, for example an empty HTTP 200 stream. The attempt then fails with <code>provider_server_error</code> and the message <code>Provider returned an empty response</code>; it is not retried and may cascade to the next child. <code>usage</code> and <code>done</code> events never commit a response on their own.

One reply without content is not empty: a reply that the token limit cut off before any text or tool call, which ends with finish reason <code>length</code> (typically a model that spent its budget reasoning), is returned as a completion with that finish reason, because the finish reason tells the caller to raise its token limit. A cascade child whose retry policy sets <code>lengthWithoutContentFails</code> treats that reply as an empty response instead, so the next child is tried; when every child fails that way, the tier ends with <code>tier_exhausted</code> like any tier whose children all returned empty responses. The free-model execution policy sets it (DS002), because in a cascade the gateway, not the caller, chose the model, and the next child may answer within the caller's limit. A direct request always receives the length finish, because no other model would be tried.
...
| 429 with a daily or billing quota signal | <code>provider_quota_exhausted</code> (HTTP 429), account-scoped. A quota signal is an <code>insufficient_quota</code> or <code>billing_hard_limit_reached</code> error type, <code>x-ratelimit-remaining: 0</code> with a reset more than five minutes away, or, on a [free-only provider](../wiki.html#definition-free-only-provider) only, a message matching free-models-per-day, per-day, daily limit or quota, insufficient credits, or billing. The message test is limited to free-only providers because there every model draws on one account allowance; on any other provider the same words also describe one model's own limit, such as a per-model requests-per-day limit, which must not lock the whole account. The account is marked exhausted until <code>x-ratelimit-reset</code>, clamped to between one minute and 26 hours; without a reset header, for at most 15 minutes (or until the next UTC midnight if sooner), after which the account is tried again. No model cooldown is recorded. |
...
A [Ploinky agent model](../wiki.html#definition-ploinky-agent-model) can report a failure only in-band once HTTP 200 has been sent, because the agent's server commits a streamed response before the agent's command handler runs. The Ploinky agent backend therefore treats an SSE frame or a JSON body that has an <code>error</code> member and no <code>choices</code> as a failed attempt. A numeric <code>error.status</code> or <code>error.code</code> from 400 to 599 is classified like that upstream status, so a handler that reports 429 produces a rate-limit failure; without one the failure is a bad-gateway server error. A streamed agent response that ends without <code>[DONE]</code> is complete only if a <code>finish_reason</code> was received; otherwise the attempt fails as truncated (<code>stream_truncated</code>), and text already delivered is followed by the route's error event. A handler must not report 404 for a transient condition, because a model-not-found failure does not cascade.
