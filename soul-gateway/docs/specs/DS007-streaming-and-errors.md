...
One reply without content is not empty: a reply that the token limit cut off before any text or tool call, which ends with finish reason <code>length</code> (typically a model that spent its budget reasoning), is returned as a completion with that finish reason, because the finish reason tells the caller to raise its token limit. A model whose retry policy sets <code>lengthWithoutContentFails</code> treats that reply as an empty response instead. The free-model execution policy sets it (DS002), because there the gateway, not the caller, chose the model, and the next child may answer within the caller's limit.
