...
- **`sessions`** — persistent conversation-group rows used by the request path and dashboard session browsers. Rows are created for live traffic by the session resolver and can also be backfilled from `main`-branch `call_logs` during historical import.
