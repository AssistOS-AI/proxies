...
const REGISTER_CONCURRENCY = parseInt(process.env.REGISTER_CONCURRENCY ?? '10', 10);
...
if (!SOUL_API_KEY) {
  ...
  console.error('       It is the sk-soul-... bearer used to call /v1/chat/completions.');
  console.error('       Create one in the dashboard (Keys tab) or via POST /management/keys.');
  process.exit(2);
}
...
class GatewayClient {
  ...
  async discoverModels(providerId) {
    const res = await this._adminCall('POST', `/management/providers/${providerId}/discover-models`, {});
    if (!res.ok) {
      const text = await res.text();
      throw new Error(`discoverModels: HTTP ${res.status} ${text.slice(0, 200)}`);
    }
    return (await res.json()).data || [];
  }
  ...
  async forceRuntimeRescan() {
    const res = await this._adminCall('POST', '/management/providers/rescan', {});
    if (!res.ok) {
...
async function main() {
  ...
  console.log(`[nvidia-sweep] register-concurrency=${REGISTER_CONCURRENCY}  test-concurrency=${CONCURRENCY}`);
  console.log(`[nvidia-sweep] timeout=${TIMEOUT_MS}ms  max_tokens=${MAX_TOKENS}  snapshot-wait=${SNAPSHOT_WAIT_MS}ms`);
  ...
  if (!nvidia) {
    console.error('No provider with provider_key="nvidia" found in /management/providers.');
    console.error('Add the NVIDIA preset from the dashboard first, or set its api_key via PATCH.');
    process.exit(3);
  }
  ...
  if (MODEL_FILTER.length > 0) {
    ...
    if (missing.length > 0) {
      console.warn(`  warning: ${missing.length} filter ids not in NVIDIA discovery: ${missing.join(', ')}`);
    }
  }
  const modelIds = candidates.slice(0, MAX_MODELS);

  console.log(`  discovered  : ${allIds.length} models${filterTag}${MAX_MODELS < Infinity && MODEL_FILTER.length === 0 ? ` (testing first ${modelIds.length})` : ''}`);
  console.log();

  if (modelIds.length === 0) {
    console.error('No models to test. Check that the NVIDIA provider has a valid api_key and your MODEL_FILTER matches.');
    process.exit(4);
  }
  ...
  console.log(`[nvidia-sweep] phase 1/4: registering ${modelIds.length} models …`);
  let registeredCount = 0;
  const registrations = await runWithConcurrency(modelIds, REGISTER_CONCURRENCY, async (providerModelId) => {
    const r = await registerOne(client, nvidia.id, providerModelId, runId);
    registeredCount += 1;
    if (registeredCount % 25 === 0 || registeredCount === modelIds.length) {
      console.log(`  registered ${registeredCount}/${modelIds.length}`);
    }
    return r;
  });
  const successfullyRegistered = registrations.filter((r) => !r.registerError);
  ...
  console.log(`  registered : ${successfullyRegistered.length} / ${modelIds.length}`);
  ...
  console.log('[nvidia-sweep] phase 2/4: forcing synchronous runtime snapshot rebuild …');
  try {
    const rescan = await client.forceRuntimeRescan();
    console.log(`  snapshot generation: ${rescan.snapshotGeneration ?? '?'}`);
  } catch (err) {
    console.warn(`  rescan failed (${err.message}); falling back to fixed wait`);
  }
  ...
  console.log(`[nvidia-sweep] phase 3/4: testing ${successfullyRegistered.length} models …`);
  let tested = 0;
  const testResults = await runWithConcurrency(successfullyRegistered, CONCURRENCY, async (registered) => {
    const r = await testOne(registered);
    tested += 1;
    const tag = r.ok ? 'PASS' : 'FAIL';
    const latency = lpad(`${r.latencyMs}ms`, 7);
    const tail = r.ok ? r.replyPreview : r.error;
    console.log(`  [${lpad(tested, 3)}/${successfullyRegistered.length}] ${tag}  ${pad(r.providerModelId, 60)}  ${latency}  ${tail}`);
    return r;
  });
  console.log();
  ...
  // ── Phase 7: cleanup ────────────────────────────────────────────
  if (!KEEP_MODELS) {
    console.log(`[nvidia-sweep] phase 4/4: deleting ${successfullyRegistered.length} temporary model rows …`);
    let deleted = 0;
    let deleteFailures = 0;
    await runWithConcurrency(successfullyRegistered, REGISTER_CONCURRENCY, async (registered) => {
      try {
      ...
      if (deleted % 25 === 0 || deleted + deleteFailures === successfullyRegistered.length) {
        console.log(`  deleted ${deleted}/${successfullyRegistered.length}${deleteFailures ? ` (${deleteFailures} failed)` : ''}`);
      }
    });
    console.log();
  }
  ...
  if (passing.length > 0) {
    const avg = (passing.reduce((acc, r) => acc + r.latencyMs, 0) / passing.length).toFixed(0);
    const min = Math.min(...passing.map((r) => r.latencyMs));
    const max = Math.max(...passing.map((r) => r.latencyMs));
    console.log(`  latency on passing models: avg=${avg}ms  min=${min}ms  max=${max}ms`);
  }
  ...
  if (KEEP_MODELS) {
    console.log();
    console.log(`[nvidia-sweep] KEEP_MODELS=1 — registered rows left in DB with key prefix "nvidia-sweep-${runId}/"`);
    console.log('  delete them with:');
    console.log(`    psql -c "DELETE FROM soul_gateway.models WHERE model_key LIKE 'nvidia-sweep-${runId}/%'"`);
  }
