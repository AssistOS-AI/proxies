...
`lib/credential.mjs` must be the only file that holds the bundled key, and it must hold it XOR-masked, never in plain text, unmasking it when the module loads so pattern-based secret scanners do not match it; the masking is not a confidentiality control. It must never be imported by browser code or by model-listing code, and the key must never appear in process arguments, logs, model rows or test output. A test must scan the repository files and require that none contains the bundled key by value or any other plaintext OpenCode key.
